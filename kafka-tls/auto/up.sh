...
docker volume create --name zk-tls-data > /dev/null
docker volume create --name zk-tls-txn-logs > /dev/null
docker volume create --name kafka-tls-data > /dev/null

# Don't need kafka-tools to start up
docker-compose up --detach --scale kafka-tools-tls=0
